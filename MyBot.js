const hlt = require('./hlt/7');
const { Direction } = require('./hlt/7/positionals');
const { Position } = require('./hlt/7/positionals');
const logging = require('./hlt/7/logging');
...
game.initialize().then(async () => {
  ...
  */
  ...
  while (true) {
    ...
    const fullness=game.turnNumber>30?fullCheck():0;
    const commandQueue = [];
    var creatd=0;
    const extra=[];
    ...
    const rnd=Math.floor(4*Math.random());
    for (const ship of me.getShips()) {
      logging.debug(ship.id,fullness,rnd);
      ...
      if(ship.haliteAmount>=Math.floor(gameMap.get(ship.position).haliteAmount*0.1)){
        ...
        if(game.turnNumber>0.92*hlt.constants.MAX_TURNS){
          ...
          //If ship is full goto shipyard
          else if(sy[ship.id]===1 || ship.haliteAmount>=maxCollect){
            if(fullness===5){
              logging.debug(ship.position.equals(me.shipyard.position.directionalOffset(Direction.getAllCardinals()[rnd])),ship.position,me.shipyard.position.directionalOffset(Direction.getAllCardinals()[rnd]));
              if(ship.position.equals(me.shipyard.position.directionalOffset(Direction.getAllCardinals()[rnd]))){
                safeMove = gameMap.getUnsafeMoves(ship.position, me.shipyard.position);
                commandQueue.push(ship.move(safeMove[0]));
                sy[ship.id]=1;
              }
            }
            else{
              destination = cship;
              safeMove = gameMap.naiveReturn(ship, destination);
              commandQueue.push(ship.move(safeMove));
              sy[ship.id]=1;
            }
          }
          //Else find best move using proSearch()
            ...
            if(mehaliteAmount>4000 && creatd===0 && gameMap.calculateDistance(ship.position,cship)>maxDO && gameMap.calculateDistance(ship.position,closestO(ship))>2){
              i=gameMap.proSearch(ship,rangePS,cship,0,fl);
            }
            //Search without dropoff condition
            else{
              i=gameMap.proSearch(ship,rangePS,cship,1,fl);
            }
            ...
            if(i===4){
              ...
              if(fullness===5 && ship.position.equals(me.shipyard.position)){
                destination = ship.position.directionalOffset(Direction.getAllCardinals()[rnd]);
                safeMove = gameMap.getUnsafeMoves(ship.position, destination);
                logging.debug(ship.move(safeMove[0]));
                commandQueue.push(ship.move(safeMove[0]));
              }
              else{
                destination = ship.position.directionalOffset(Direction.getAllCardinals()[i]);
                safeMove = gameMap.naiveNavigate(ship, destination);
                logging.debug(ship.move(safeMove));
                commandQueue.push(ship.move(safeMove));
              }
            }
          }
    ...
    if ((mehaliteAmount >= 1000 && tot<cs && !gameMap.get(me.shipyard).isOccupied && fullness<3) || game.turnNumber<6) {
